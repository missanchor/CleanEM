...
1. Do NOT flag values just because they're not in top_values - these are SAMPLES only unless unique_count is small (<20)
...
Example (detect 'x' substitution typo):
lambda value: 'x' not in str(value).lower() if value else True
"""
...
- Top categorical values (SAMPLES ONLY - not exhaustive): {json.dumps(categorical_values, ensure_ascii=False)}
- Min: {min_val}, Max: {max_val}, Mean: {mean_val}, Std: {std_val}
- Numeric count: {numeric_count}

IMPORTANT GUIDELINES:
1. Do NOT flag values just because they're not in top_values - these are SAMPLES only unless unique_count is small (<50)
2. For categorical: look for characteristic patterns (value structure, format) rather than exact value lists
3. For numeric: check reasonable value ranges based on statistics with tolerance
4. For text: check reasonable length and character constraints
5. Be PERMISSIVE - only reject clearly inaccurate values

Return ONLY lambda functions, one per line. Format:
lambda value, row=None: <expression>

Example for numeric 'age' (with tolerance):
lambda value, row=None: value is not None and str(value).strip() not in ['', 'nan', 'none', 'null', 'n/a', 'na', 'unknown'] and str(value).replace('.', '', 1).replace('-', '', 1).isdigit() and -1000 <= float(value) <= 5000

Example for categorical 'status' (if exhaustive enumeration):
